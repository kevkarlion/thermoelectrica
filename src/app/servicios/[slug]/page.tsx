...
const SERVICES_DATA: any = {
  "hvac-climatizacion-industrial": {
    ...
    features: ["Instalación de Chillers", "Mantenimiento UMA", "Balanceo de caudales", "Sistemas VRV"]
  },
  "refrigeracion-industrial": {
    ...
    features: ["Centrales de frío", "Túneles IQF", "Compresores de tornillo", "Optimización energética"]
  },
  "refrigeracion-precision-data-centers": {
    ...
    features: ["Unidades CRAC", "Contención de pasillos", "Auditorías térmicas", "Respuesta inmediata"]
  },
  "sistemas-refrigeracion-amoniaco-nh3": {
    ...
    features: ["Certificación de recipientes", "Válvulas de seguridad", "Capacitación", "Auditorías IIAR"]
  },
  "ingenieria-electrica-industrial": {
    ...
    features: ["Tableros de transferencia", "Análisis de armónicos", "Canalización robusta", "Automatización"]
  },
  "mantenimiento-predictivo-analisis": {
    ...
    features: ["Análisis termográfico", "Medición vibracional", "Análisis de aceite", "Informes técnicos"]
  }
};
...
export default async function Page({ params }: Props) {
  ...
  // Debug para consola de servidor
  console.log("Cargando slug:", slug);

  const data = SERVICES_DATA[slug];

  if (!data) {
    console.error("Slug no encontrado en SERVICES_DATA:", slug);
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-white py-20">
      <div className="container-custom">
        <Link href="/servicios" className="flex items-center gap-2 text-primary font-bold mb-10 hover:text-accent transition-colors">
          <ArrowLeft size={20} /> VOLVER A SERVICIOS
        </Link>
        
        <div className="max-w-4xl">
          <span className="text-accent font-black tracking-widest uppercase text-sm">Ingeniería Especializada</span>
          <h1 className="text-5xl md:text-7xl font-black text-gray-950 mt-4 mb-6 uppercase italic italic tracking-tighter italic leading-none">
            {data.title}
          </h1>
          <p className="text-2xl text-primary font-bold mb-8 italic">{data.subtitle}</p>
          <p className="text-gray-600 text-xl leading-relaxed mb-12">{data.description}</p>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 bg-gray-50 p-8 rounded-3xl border border-gray-100 mb-16">
            {data.specs.map((spec: any, i: number) => (
              <div key={i}>
                <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-1">{spec.label}</p>
                <p className="text-sm font-bold text-gray-900 uppercase">{spec.value}</p>
              </div>
            ))}
          </div>
